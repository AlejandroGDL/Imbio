...
interface InjustificantesListProps {
  personalId?: number;
  personalList?: Personal[];
  onEdit: (item: Injustificante) => void;
...
export function InjustificantesList({
  personalId: personalIdForzado,
  personalList = [],
  onEdit,
  refreshTrigger = 0,
}: InjustificantesListProps) {
  const { status } = useServerStatus();
  const [items, setItems] = useState<Injustificante[]>([]);
  ...
  const load = async () => {
    ...
    try {
      ...
      const res = await api.listarInjustificantes({
        q: search || undefined,
      ...
      setError(
        err instanceof ApiError
          ? err.message
          : "Error al cargar injustificantes",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (i: Injustificante) => {
    setDeletingId(i.id);
    try {
      await api.eliminarInjustificante(i.id);
      const empName = i.personal
        ? `${i.personal.nombre} ${i.personal.apellidos}`
        : `Empleado #${i.personalId}`;
      toast.success("Injustificante eliminado", {
        description: `${empName} — ${formatFecha(i.fecha)}`,
      });
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <span>
            {personalIdForzado
              ? "Injustificantes del empleado"
              : "Injustificantes registrados"}
          </span>
        ...
        <CardDescription>
          {status !== "online"
            ? "Conecta el servidor para ver los injustificantes."
            : "Busca por empleado o razón. Filtra por rango de fechas."}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
          ...
          >
            <div className="relative flex-1">
              ...
              <Input
                ...
                placeholder="Buscar por empleado, puesto o razón…"
                className="pl-9 pr-9"
                autoComplete="off"
              />
              {searchInput && (
        ...
        {!loading && !error && status === "online" && items.length === 0 && (
          <div className="py-12 text-center text-muted-foreground">
            ...
            <p>
              {hayFiltros
                ? "No se encontraron injustificantes con esos filtros."
                : personalIdForzado
                  ? "Este empleado no tiene injustificantes registrados."
                  : "Aún no hay injustificantes registrados."}
            </p>
            {hayFiltros && (
              <Button
                type="button"
            ...
            {!hayFiltros && !personalIdForzado && (
              <p className="mt-1 text-sm">
                Usa el botón "Nuevo injustificante" para registrar el primero.
              </p>
            )}
          </div>
        )}
                ...
                {items.map((i) => (
                  <TableRow key={i.id}>
                    ...
                    <TableCell className="max-w-[480px]">
                      <div className="flex items-start gap-1 text-sm text-rose-700">
                        <AlertTriangle className="mt-0.5 h-3 w-3 shrink-0" />
                        <span className="line-clamp-2" title={i.razon}>
                          <Highlight text={i.razon} query={search} />
                        </span>
                      </div>
                    </TableCell>
    ...
    <AlertDialog>
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogTitle className="text-center">¿Eliminar injustificante?</AlertDialogTitle>
